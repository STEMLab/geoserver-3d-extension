 ...
 *
 */
public class GetCapabilities3D {
    /**
     ...
     */
    public GetCapabilities3D(WFSInfo wfs, Catalog catalog, Collection<WFSExtendedCapabilitiesProvider> extendedCapabilitiesProviders) {
        this.wfs = wfs;
        this.catalog = catalog;
        this.extendedCapabilitiesProviders = extendedCapabilitiesProviders;
    }

    public CapabilitiesTransformer3D run(GetCapabilitiesRequest3D request)
        throws WFSException {
        ...
        final CapabilitiesTransformer3D capsTransformer;
        if ("1.0.0".equals(version)) {
            capsTransformer = new CapabilitiesTransformer3D.WFS1_0(wfs, catalog);
        } else {
            if ("1.1.0".equals(version)) {
                capsTransformer = new CapabilitiesTransformer3D.WFS1_1(wfs, baseUrl, catalog,
                        extendedCapabilitiesProviders);
            } else if ("2.0.0".equals(version)) {
                capsTransformer = new CapabilitiesTransformer3D.WFS2_0(wfs, baseUrl, catalog,
                        extendedCapabilitiesProviders);
            } else {
    ...
    public static String version(GetCapabilitiesRequest3D request) {
        // do the version negotiation dance
        List<String> provided = new ArrayList<String>();
        provided.add("1.0.0");
        provided.add("1.1.0");

        if (request instanceof GetCapabilitiesRequest3D.WFS20) {
            provided.add("2.0.0");    
        }
