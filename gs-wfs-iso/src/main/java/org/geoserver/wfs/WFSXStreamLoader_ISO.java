 ...
 *
 */
public class WFSXStreamLoader_ISO extends XStreamServiceLoader<WFSInfo> {

    public WFSXStreamLoader_ISO(GeoServerResourceLoader resourceLoader) {
        super(resourceLoader, "wfs3d");
    }
    ...
    public static void initXStreamPersister(XStreamPersister xp) {
        ...
        xs.alias( "wfs3d", WFSInfo.class, WFSInfoImpl_ISO.class );
    ...
    protected WFSInfo initialize(WFSInfo service) {
        ...
        if(service.getGML() == null) {
            ((WFSInfoImpl_ISO) service).setGML(new HashMap<WFSInfo.Version, GMLInfo>());           
        }
        ...
        if (service.getSRS() == null) {
            ((WFSInfoImpl_ISO) service).setSRS(new ArrayList<String>());
        }
        return service;
    }
