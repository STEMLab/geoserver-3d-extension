...
import org.geoserver.feature.ISOCompositeFeatureCollection;
 ...
 * @generated
 */
public class ISOFeatureCollectionTypeBinding extends AbstractComplexEMFBinding {
    ...
    public ISOFeatureCollectionTypeBinding(WfsFactory wfsfactory, Catalog catalog,
    ...
    public ISOFeatureCollectionTypeBinding(WfsFactory wfsfactory, Catalog catalog,
        ...
        this.generateBounds = !configuration.getProperties().contains(GMLConfiguration_ISO.NO_FEATURE_BOUNDS);
        this.encodeFeatureMember = configuration.getProperties().contains(GMLConfiguration_ISO.ENCODE_FEATURE_MEMBER);
    }
        ...
        if (GML.featureMembers.equals(name)) {
            ...
            for (Iterator it = featureCollection.getFeature().iterator(); it.hasNext();) {
                ...
                if(env != null &&  env.getCoordinateReferenceSystem() == null) {
                    ...
                    env = ReferencedEnvelope.create(env, crs);
                }
    ...
    private Object handleFeatureCollection(FeatureCollectionType featureCollection) {
        ...
        if (featureCollection.getFeature().size() > 1) {
            // wrap in a single
            result = new ISOCompositeFeatureCollection(featureCollection.getFeature());
        } else {
        ...
        if (isSimpleFeatureCollection(result)
                && encoder.getConfiguration().hasProperty(
                        GMLConfiguration_ISO.OPTIMIZED_ENCODING)) {
    ...
    private boolean isSimpleFeatureCollection(FeatureCollection result) {
        ...
        if (result instanceof ISOCompositeFeatureCollection) {
        	ISOCompositeFeatureCollection composite = (ISOCompositeFeatureCollection) result;
