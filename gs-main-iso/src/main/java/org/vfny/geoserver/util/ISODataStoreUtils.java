...
import org.geoserver.feature.retype.ISORetypingDataStore;
 ...
 * @version $Id$
 */
public abstract class ISODataStoreUtils {
            ...
            try {
                ...
                for (int i = 0; i < names.length; i++) {
                    if (names[i].indexOf(":") >= 0)
                        return new ISORetypingDataStore((DataStore) store);
                }
    ...
    public static<K,V> Map<K,V> getParams(Map<K,V> m, ServletContext sc) {
        ...
        return ISOResourcePool.getParams(m, loader );
    }
