 ...
 * 
 */
public class ISOCatalogBuilder {

    static final Logger LOGGER = Logging.getLogger(ISOCatalogBuilder.class);
    ...
    public ISOCatalogBuilder(Catalog catalog) {
    ...
    public ReferencedEnvelope getLatLonBounds(ReferencedEnvelope nativeBounds,
        ...
        if (nativeBounds != null && declaredCRS != null) {
            ...
            if (!CRS.equalsIgnoreMetadata(DefaultGeographicCRS.WGS84, declaredCRS)) {
                // transform
                try {
                    ReferencedEnvelope bounds = ReferencedEnvelope.create(nativeBounds, CRS.getHorizontalCRS(declaredCRS));
                    return bounds.transform(DefaultGeographicCRS.WGS84, true);
                } catch (Exception e) {
                    throw (IOException) new IOException("transform error").initCause(e);
                }
            } else {
                return ReferencedEnvelope.create(nativeBounds, DefaultGeographicCRS.WGS84);
            }
        }
        return null;
    }
    ...
    ReferencedEnvelope getNativeBounds(ResourceInfo rinfo, Object data) throws IOException {
        ...
        if (rinfo instanceof FeatureTypeInfo) {
            ...
            if (bounds != null && bounds.getCoordinateReferenceSystem() == null && crs != null) {
                bounds = ReferencedEnvelope.create(bounds, crs);
            }
            ...
            // get  bounds
            bounds = ReferencedEnvelope.reference(reader.getOriginalEnvelope());
    ...
    public void initCoverage(CoverageInfo cinfo, final String coverageName) throws Exception {
    	...
    	if (cinfo.getLatLonBoundingBox() == null
    		...
    		cinfo.setNativeBoundingBox(ReferencedEnvelope.reference(envelope));
    		cinfo.setLatLonBoundingBox(ReferencedEnvelope.reference(CoverageStoreUtils.getWGS84LonLatEnvelope(envelope)));
    ...
    public StyleInfo getDefaultStyle(ResourceInfo resource) throws IOException {
        ...
        } else if (Curve.class.isAssignableFrom(gtype)
                || MultiCurve.class.isAssignableFrom(gtype)) {
            styleName = StyleInfo.DEFAULT_LINE;
        } else if (Surface.class.isAssignableFrom(gtype)
                || MultiSurface.class.isAssignableFrom(gtype)) {
    ...
    public ReferencedEnvelope getBoundsFromCRS(ResourceInfo resource) {
        ...
        if (crs != null) {
            ...
            if (crsEnvelope != null) {
                crsReferencedEnvelope = ReferencedEnvelope.reference(crsEnvelope);    
            } 
        }
