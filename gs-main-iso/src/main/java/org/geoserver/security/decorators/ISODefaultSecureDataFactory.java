 ...
 * 
 */
public class ISODefaultSecureDataFactory implements SecuredObjectFactory {
    ...
    public Object secure(Object object, WrapperPolicy policy) {
        ...
        if (DataStore.class.isAssignableFrom(clazz)) {
            return new ISOReadOnlyDataStore((DataStore) object, policy);
        ...
        if (SimpleFeatureSource.class.isAssignableFrom(clazz)) {
            if ((policy.level == AccessLevel.READ_ONLY || policy.level == AccessLevel.METADATA
                ...
                return new ISOSecuredSimpleFeatureSource((SimpleFeatureSource) object, policy);
            } else if (SimpleFeatureLocking.class.isAssignableFrom(clazz)) {
                return new ISOSecuredSimpleFeatureLocking((SimpleFeatureLocking) object, policy);
            } else if (SimpleFeatureStore.class.isAssignableFrom(clazz)) {
                return new ISOSecuredSimpleFeatureStore((SimpleFeatureStore) object, policy);
            ...
            if ((policy.level == AccessLevel.READ_ONLY || policy.level == AccessLevel.METADATA 
                    || policy.level == AccessLevel.HIDDEN) && policy.response != Response.CHALLENGE) {
                return new ISOSecuredFeatureSource((FeatureSource) object, policy);
            } else if (FeatureLocking.class.isAssignableFrom(clazz)) {
                return new ISOSecuredFeatureLocking((FeatureLocking) object, policy);
            } else if (FeatureStore.class.isAssignableFrom(clazz)) {
                return new ISOSecuredFeatureStore((FeatureStore) object, policy);
            } else if (FeatureSource.class.isAssignableFrom(clazz)) {
                return new ISOSecuredFeatureSource((FeatureSource) object, policy);
            }
        }
        ...
        if (SimpleFeatureCollection.class.isAssignableFrom(clazz)) {
            return new ISOSecuredSimpleFeatureCollection((SimpleFeatureCollection) object, policy);
