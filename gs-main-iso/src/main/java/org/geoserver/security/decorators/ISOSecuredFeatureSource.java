 ...
 * @param <F>
 */
public class ISOSecuredFeatureSource<T extends FeatureType, F extends Feature>
        extends DecoratingFeatureSource<T, F> {
    
    static final Logger LOGGER = Logging.getLogger(ISOSecuredFeatureSource.class);

    WrapperPolicy policy;

    protected ISOSecuredFeatureSource(FeatureSource<T, F> delegate, WrapperPolicy policy) {
        super(delegate);
        this.policy = policy;
    }
        ...
        if (fc == null) {
            ...
            if (limitedAttributeSize > 0 && fc.getSchema().getDescriptors().size() > limitedAttributeSize) {
                if(fc instanceof SimpleFeatureCollection) {
                    ...
                    SimpleFeatureType target = ISOSimpleFeatureTypeBuilder.retype(sfc.getSchema(), mixed.getPropertyNames());
                    ISOReTypingFeatureCollection retyped = new ISOReTypingFeatureCollection(sfc, target);
    ...
    protected Query mixQueries(Query userQuery, Query securityQuery) {
        // first rough mix
        Query result = ISODataUtilities.mixQueries(userQuery, securityQuery, userQuery.getHandle());
