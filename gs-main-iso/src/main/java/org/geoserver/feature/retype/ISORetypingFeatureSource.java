 ...
 * {@link ISORetypingDataStore}
 */
public class ISORetypingFeatureSource implements SimpleFeatureSource{

    SimpleFeatureSource wrapped;

    FeatureTypeMap typeMap;

    ISORetypingDataStore store;
    ...
    public static SimpleFeatureSource getRetypingSource(SimpleFeatureSource wrapped, SimpleFeatureType targetSchema) throws IOException {
        ...
        if(wrapped instanceof SimpleFeatureLocking) {
            return new ISORetypingFeatureLocking((SimpleFeatureLocking) wrapped, map);
        } else if(wrapped instanceof SimpleFeatureStore) {
            return new ISORetypingFeatureStore((SimpleFeatureStore) wrapped, map);
        } else {
            return new ISORetypingFeatureSource(wrapped, map);
        }
        
    }

    ISORetypingFeatureSource(ISORetypingDataStore ds,
    ...
    ISORetypingFeatureSource(SimpleFeatureSource wrapped, final FeatureTypeMap typeMap) throws IOException {
        this.wrapped = wrapped;
        this.typeMap = typeMap;
        this.store = new ISORetypingDataStore((DataStore) wrapped.getDataStore()) {
            @Override
    ...
    public SimpleFeatureCollection getFeatures(Query query) throws IOException {
        ...
        return new ISOReTypingFeatureCollection(wrapped.getFeatures(store.retypeQuery(query, typeMap)),
                target);
    }
