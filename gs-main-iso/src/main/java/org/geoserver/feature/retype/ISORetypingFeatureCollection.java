...
public class ISORetypingFeatureCollection extends ISODecoratingSimpleFeatureCollection {
    ...
    public ISORetypingFeatureCollection(SimpleFeatureCollection delegate,
            SimpleFeatureType target) {
        super(delegate);
        this.target = target;
    }
    ...
    protected boolean canDelegate(FeatureVisitor visitor) {
        return ISOReTypingFeatureCollection.isTypeCompatible(visitor, target);
    }
    ...
    public static class RetypingIterator implements SimpleFeatureIterator {
        ...
        public SimpleFeature next() {
            try {
                return ISORetypingFeatureCollection.retype(delegate.next(), builder);
    ...
    public static class ISORetypingFeatureReader implements
            FeatureReader<SimpleFeatureType, SimpleFeature> {
        FeatureReader<SimpleFeatureType, SimpleFeature> delegate;
        ISOSimpleFeatureBuilder builder;

        public ISORetypingFeatureReader(FeatureReader<SimpleFeatureType, SimpleFeature> delegate,
                SimpleFeatureType target) {
            this.delegate = delegate;
            this.builder = new ISOSimpleFeatureBuilder(target);
        }
    ...
    public static class RetypingFeatureWriter implements
        ...
        public SimpleFeature next() throws IOException {
            try {
                ...
                retyped = ISORetypingFeatureCollection.retype(current, builder);
                return retyped;
            } catch (IllegalAttributeException e) {
